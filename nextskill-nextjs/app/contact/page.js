...
// --- Header component copied from video-tutorials/page.js ---
function Header() {
...
// --- Footer component copied from video-tutorials/page.js ---
function Footer() {
  return (
...
export default function ContactPage() {
  // FAQ state (optional, for expand/collapse)
    ...
    <main className="min-h-screen flex flex-col bg-[#f5f8fe]">
      ...
      <section className="w-full bg-gradient-to-br from-blue-50 to-blue-100 border-b border-gray-200 py-12 mb-8">
        <div className="max-w-5xl mx-auto px-4 flex flex-col md:flex-row md:items-center md:gap-10">
          <div className="flex-1">
            <h1 className="text-4xl font-extrabold text-gray-900 mb-4">Contact NextSkill Cambodia</h1>
            <p className="text-lg text-gray-700 mb-6">We're here to help with your learning journey and answer any questions. Reach out to us anytime!</p>
          </div>
          <div className="flex-1 flex items-center justify-center mt-8 md:mt-0">
            <span className="text-6xl text-blue-600 opacity-80"><i className="fas fa-envelope-open-text"></i></span>
          </div>
        </div>
      </section>
      {/* Main Content */}
      <section className="max-w-5xl mx-auto px-4 pb-16">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          <div className="flex items-center gap-4 bg-blue-50 rounded-xl p-6 border-l-4 border-blue-700">
        ...
        <div className="bg-white rounded-2xl shadow-md p-8 mb-12">
          <h2 className="text-2xl font-bold flex items-center gap-3 mb-6 text-gray-900"><i className="fas fa-paper-plane text-blue-700"></i> Send Us a Message</h2>
          <form className="grid gap-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block font-semibold text-gray-700 mb-2">Name</label>
                <input type="text" className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-blue-600 focus:border-blue-700 text-gray-900 bg-gray-50" placeholder="Your Name" />
              </div>
              <div>
                <label className="block font-semibold text-gray-700 mb-2">Email</label>
                <input type="email" className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-blue-600 focus:border-blue-700 text-gray-900 bg-gray-50" placeholder="you@email.com" />
              </div>
            </div>
            <div>
              <label className="block font-semibold text-gray-700 mb-2">Message</label>
              <textarea className="w-full rounded-lg border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-blue-600 focus:border-blue-700 text-gray-900 bg-gray-50 min-h-[120px]" placeholder="How can we help you?"></textarea>
            </div>
            <button type="submit" className="bg-blue-700 hover:bg-blue-800 text-white font-semibold px-8 py-3 rounded-lg shadow transition w-full md:w-auto">Send Message</button>
          </form>
        </div>
        {/* Social Links */}
        <div className="flex gap-4 justify-center mb-12">
          <a href="https://t.me/nextskillcambodia" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#0088cc] text-white rounded-full flex items-center justify-center text-2xl hover:shadow-lg transition social-link telegram"><i className="fab fa-telegram-plane"></i></a>
          <a href="https://facebook.com/nextskillcambodia" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#1877f2] text-white rounded-full flex items-center justify-center text-2xl hover:shadow-lg transition social-link facebook"><i className="fab fa-facebook-f"></i></a>
          <a href="https://youtube.com/@nextskillcambodia" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#ff0000] text-white rounded-full flex items-center justify-center text-2xl hover:shadow-lg transition social-link youtube"><i className="fab fa-youtube"></i></a>
        </div>
        {/* FAQ Section (optional, add if you want) */}
        {/* <div className="bg-white rounded-2xl shadow-md p-8 mb-12 faq-section">
          <h2 className="text-2xl font-bold flex items-center gap-3 mb-6 text-gray-900"><i className="fas fa-question-circle text-blue-700"></i> Frequently Asked Questions</h2>
          <div className="faq-item">
            <button className="faq-question" onClick={() => setOpenFaq(openFaq === 0 ? null : 0)}>
              How do I enroll in a course?
              <span className={`faq-toggle ${openFaq === 0 ? 'active' : ''}`}>▼</span>
            </button>
            <div className={`faq-answer ${openFaq === 0 ? 'active' : ''}`}>Simply browse our courses and click "Start Learning" to enroll for free. No registration required!</div>
          </div>
          <div className="faq-item">
            <button className="faq-question" onClick={() => setOpenFaq(openFaq === 1 ? null : 1)}>
              Is all content really free?
              <span className={`faq-toggle ${openFaq === 1 ? 'active' : ''}`}>▼</span>
            </button>
            <div className={`faq-answer ${openFaq === 1 ? 'active' : ''}`}>Yes! All our video tutorials and resources are 100% free for everyone.</div>
          </div>
        </div> */}
      </section>
      <Footer />
